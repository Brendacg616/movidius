...
void ActionDetection::fetchResults(InferRequest::Ptr request) {
    /*if (!enabled()) return;
    results.clear();
...
*/
    ...
    /** Parse detections **/
    //std::cout<<"Before GetDetections"<<std::endl;
...
ActionDetection::ParseBBoxRecord(
    ...
    bbox.ymax = data[3];
    //std::cout<<"bbox.ymax: "<<bbox.xmax<<std::endl;
    return bbox;
}
...
cv::Rect ActionDetection::ConvertToRect(
    ...
    std::cout<<"prior_bboxxmax "<<prior_bbox.xmax<<std::endl;
    std::cout<<"prior_bboxxmin "<<prior_bbox.xmin<<std::endl;
    std::cout<<"prior_bboxymax "<<prior_bbox.ymax<<std::endl;
    std::cout<<"prior_bboxymin "<<prior_bbox.ymin<<std::endl;
    ...
    std::cout<<"prior_width "<<prior_width<<std::endl;
    std::cout<<"prior_height "<<prior_height<<std::endl;
    /** Create decoded bbox*/
    ...
    std::cout<<"decoded_bbox.xmin: "<<decoded_bbox.xmin<<std::endl;
    std::cout<<"decoded_bbox.ymin: "<<decoded_bbox.ymin<<std::endl;
    std::cout<<"decoded_bbox.xmax: "<<decoded_bbox.xmax<<std::endl;
    std::cout<<"decoded_bbox.ymax: "<<decoded_bbox.ymax<<std::endl;
    std::cout<<"frame_size.width "<<frame_size.width<<std::endl;
    std::cout<<"frame_size.height "<<frame_size.height<<std::endl;
    float test_width=(decoded_bbox.xmax - decoded_bbox.xmin) * frame_size.width ;
    std::cout<<"test_width "<<test_width<<std::endl;
    int test_width_int=static_cast<int>(test_width);
    std::cout<<"test_width_int "<<test_width_int<<std::endl;
...
void ActionDetection::GetDetections(const cv::Mat& loc, const cv::Mat& main_conf,
    ...
    //std::cout<<"GetDetections preiterations"<<std::endl;
    ...
    for (int p = 0; p < num_candidates; ++p) {
      ...
      //std::cout<<"num_candidates: "<<num_candidates<<std::endl;
        ...
        //  std::cout<<"det_conf_data: "<<det_conf_data<<std::endl;
        /** Skip low-confidence detections */
        if (detection_conf < config_.detection_confidence_threshold) {
            continue;
        }
        //std::cout<<"num_action_classes: "<<config_.num_action_classes<<std::endl;
        ...
        std::cout<<"Before parsebbox"<<std::endl;
...
/*
        std::cout<<"det_rect.x: "<<det_rect.x<<std::endl;
        std::cout<<"det_rect.y: "<<det_rect.y<<std::endl;
        std::cout<<"det_rect.width: "<<det_rect.width<<std::endl;
        std::cout<<"det_rect.height: "<<det_rect.height<<std::endl;
*/
        /** Store detected action **/
        valid_detections.emplace_back(det_rect, action_label, detection_conf, action_conf);
        std::cout<<"valid_detections "<<valid_detections.size()<<std::endl;
    }
    //std::cout<<"Before SoftNonMaxSuppression "<<std::endl;
    ...
    SoftNonMaxSuppression(valid_detections, config_.nms_sigma, config_.keep_top_k,
                          ...
                          &out_det_indices);
//std::cout<<"After SoftNonMaxSuppression "<<std::endl;

    detections->clear();

//std::cout<<"Before order vec "<<std::endl;
std::cout<<"out_det_indices "<<out_det_indices.size()<<std::endl;
...
void ActionDetection::SoftNonMaxSuppression(const DetectedActions& detections,
  ...
  //  std::cout<<"SoftNonMaxSuppression "<<std::endl;
